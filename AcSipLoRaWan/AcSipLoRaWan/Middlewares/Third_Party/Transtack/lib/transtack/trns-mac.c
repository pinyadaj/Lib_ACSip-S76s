...
int16_t build_pkt_trns_mac (conf_trns_t* conf_pt,
                             uint16_t    dst_addr,  
                             uint16_t    src_addr,  
                             ctrl_t      ctrl,
                             hdr_typ_t      type,
...
int16_t split_pkt_trns_mac (conf_trns_t* conf_pt,
                             int8_t* buf_pt, int16_t len);

void wait_receive_trns_mac (conf_trns_t* conf_pt);

int16_t hcs_trns_mac (conf_trns_t* conf_pt);

int16_t snd_trns_try_mac(conf_trns_t* conf_pt);

int16_t seq_chk_trns_mac(conf_trns_t* conf_pt);

int32_t tmout_calc_mac (uint32_t baud, uint16_t pdu_len);

int16_t proc_tmout_mac (conf_trns_t* conf_pt);
int16_t proc_rcv_tmout_mac (conf_trns_t* conf_pt);

int16_t xmit_trns_mac    (conf_trns_t* conf_pt,
                      int8_t*     xmit_buf_pt,
                      uint16_t     len_buf);

int16_t rcv_ind_tran_mac (conf_trns_t* conf_pt,
                      uint16_t dst_addr, 
                      uint16_t src_addr, 
...
int16_t snd_cnf_trns_mac (conf_trns_t* conf_pt,
                      uint16_t     dst_addr, 
                      uint16_t     src_addr, 
					  result_xmit_state_t result_xmit);

int16_t tmout_ind_trns_mac (conf_trns_t* conf_pt,
                        uint16_t dst_addr, 
                        uint16_t src_addr, 
                        result_rcv_state_t result_rcv);
...
int16_t init_trns_mac (conf_trns_t*       create_conf_pt,
						int8_t mode,
                       uint16_t           self_addr,
					   ...
					   rw_cb_t*       rcv_cb)
{
    ...
    create_conf_pt->trns_st = TSK_RCV_STS;
...
void process_trns_mac (conf_trns_t* conf_pt) {
    ...
    switch (conf_pt->trns_st) {
        case TSK_IDLE_STS:
             conf_pt->trns_st = TSK_RCV_STS;
            break;
                      ...
                      int16_t    len_pyl_buf)
{
    ...
    if ((hdr_type == HDR_TYP_REQR_PKT) || (hdr_type == HDR_TYP_REPR_PKT)) {
    	...
    	ret = build_pkt_trns_mac (conf_pt,
                dst_addr,
                src_addr,
                ctrl,
                hdr_type,
    ...
    if (ret == 1) {
        ret = 0;
        // transmit
        if ((ret = xmit_trns_mac (conf_pt, &conf_pt->tx_pdu_buf[0], conf_pt->tx_pdu_len)) > 0) {
...
//    	LOG_INFO ("rx_tmout = %u ms\n\r", conf_pt->rx_tmout);
     ...
     //  if (conf_pt->rx_tmout > 0) conf_pt->tk_tmout_prev = conf_pt->tk_tmout = tick_sys;
       ...
       split_pkt_trns_mac (conf_pt, &buf_tmp[0], len_tmp);
    }
            ...
            if (tmp_crc == MAKE16 (conf_pt->rx_pdu_buf[PL_CRC_PDU_OFFSET], conf_pt->rx_pdu_buf[PL_CRC_PDU_OFFSET + 1])) {
                ...
                if (ret > 0) {
                	...
                	if (conf_pt->mode == MODE_MASTER) {
                		snd_trns_try_mac(conf_pt);
                	}
                   
                }else {
  //                  ret = frcmnt_chk_trns_mac(conf_pt);
                }
            }
            else {
