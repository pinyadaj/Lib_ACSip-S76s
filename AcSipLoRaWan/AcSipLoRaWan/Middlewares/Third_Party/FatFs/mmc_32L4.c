...
//WORD d;
UINT i_cnt;
     for (i_cnt = 0; i_cnt < btr; i_cnt++) //read n bytes data
          buff[i_cnt] = xchg_spi(0xFF);
...
)
{
  //	WORD d;
    ...
    for (i_cnt = 0; i_cnt < btx; i_cnt++) //send n bytes data
          xchg_spi( buff[i_cnt] );
...
/*-----------------------------------------------------------------------*/
/* Deselect_sdcard card and release SPI                                         */
/*-----------------------------------------------------------------------*/

static
void deselect_sdcard (void)
{
	CS_HIGH();		/* Set CS# high */
...
/*-----------------------------------------------------------------------*/
/* select_sdcard card and wait for ready                                        */
/*-----------------------------------------------------------------------*/

static
int select_sdcard (void)	/* 1:OK, 0:Timeout */
{
...
)
{
	...
	/* select_sdcard the card and wait for ready except to stop multiple block read */
	if (cmd != CMD12) {
		deselect_sdcard();
	...
	do
		res = xchg_spi(0xFF); //xchg_spi(0xFF);
	while ((res & 0x80) && --n);
