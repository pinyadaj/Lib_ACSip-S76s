...
acsip_ret_t mac_get_dc_ctl_acsip (char *rep_buf, uint16_t len_max) {
	return get_acsip("mac get_dc_ctl", NULL, rep_buf, len_max, 1000);
}
...
acsip_ret_t mac_set_batt_acsip(int8_t battery) {
	char tmp_buf[128];
	sprintf(tmp_buf, "mac set_batt %u", battery);
	transfer_acsip(tmp_buf, "Ok", 1000, 0);
	return get_return_code();
}
...
acsip_ret_t mac_set_ch_count_acsip(uint8_t channelscount, uint8_t bw){
	char snd_buf[128];
	sprintf(snd_buf, "mac set_downlink_dwell %s", on ? "125": "500");
	transfer_acsip(snd_buf, "Ok", 1000, 0);
	return get_return_code();
}
...
acsip_ret_t mac_set_auto_join_acsip(uint8_t on, uint8_t join_type, uint16_t join_count){
	char snd_buf[128];
	sprintf(snd_buf, "mac set_auto_join %s", on ? "on": "off", join_type ? "otaa" : "abp", join_count);
	transfer_acsip(snd_buf, "Ok", 1000, 0);
	return get_return_code();
}
